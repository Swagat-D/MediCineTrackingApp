...
const BarcodeGeneratorScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const filteredBarcodes = useMemo(() => {
    ...
    return recentBarcodes.filter((barcode) => {
      return (
        barcode.patientName.toLowerCase().includes(query) ||
        barcode.medicationName.toLowerCase().includes(query) ||
        barcode.barcodeData.toLowerCase().includes(query) ||
        barcode.dosage.toLowerCase().includes(query) ||
        barcode.frequency.toLowerCase().includes(query) ||
        barcode.timingRelation.toLowerCase().replace('_', ' ').includes(query)
      );
    });
  }, [recentBarcodes, searchQuery]);
  ...
  const handleDownloadBarcode = async (barcode: BarcodeItem) => {
    try {
      ...
      // Create barcode data for download
      const barcodeData: BarcodeData = {
        patientName: barcode.patientName,
        barcodeData: barcode.barcodeData,
        medicationName: barcode.medicationName,
        dosage: barcode.dosage,
        frequency: barcode.frequency,
        timingRelation: barcode.timingRelation,
      };
...
Generated by MediTracker`;
  ...
  const handleBulkDownload = async () => {
    ...
    const selectedBarcodesData: BarcodeData[] = filteredBarcodes
      .filter(barcode => selectedBarcodes.includes(barcode.id))
      .map(barcode => ({
        patientName: barcode.patientName,
        barcodeData: barcode.barcodeData,
        medicationName: barcode.medicationName,
        ...
        <View style={styles.barcodeHeader}>
          <View style={styles.patientInfo}>
            ...
            <Text style={styles.dosageDetails}>
              {barcode.frequency} • {barcode.timingRelation.replace('_', ' ')}
            </Text>
          </View>
          ...
          <View style={styles.searchSection}>
            <View style={[styles.searchContainer, isSearchFocused && styles.searchContainerFocused]}>
              <Ionicons 
                name="search" 
                size={20} 
                color={isSearchFocused ? "#059669" : "#94A3B8"} 
                style={styles.searchIcon}
              />
              <TextInput
                style={styles.searchInput}
                placeholder="Search by patient, medication, or barcode..."
            ...
            {searchQuery.trim() && (
              <View style={styles.searchResults}>
                <Text style={styles.searchResultsText}>
                  {filteredBarcodes.length} result{filteredBarcodes.length !== 1 ? 's' : ''} found
                  {searchQuery.trim() && ` for "${searchQuery.trim()}"`}
                </Text>
                {filteredBarcodes.length > 0 && filteredBarcodes.length < recentBarcodes.length && (
                  <TouchableOpacity onPress={clearSearch} style={styles.showAllButton}>
                    <Text style={styles.showAllText}>Show all ({recentBarcodes.length})</Text>
                  </TouchableOpacity>
                )}
              </View>
            )}
        ...
        <View style={styles.barcodesSection}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>
              {searchQuery.trim() ? 'Search Results' : 'Recent Barcodes'}
            </Text>
            <Text style={styles.sectionSubtitle}>
              {searchQuery.trim() 
                ? `${filteredBarcodes.length} barcode${filteredBarcodes.length !== 1 ? 's' : ''} found`
            ...
            {filteredBarcodes.length === 0 ? (
              searchQuery.trim() ? (
                ...
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyMessage}>
                    Add medications to patients to generate barcodes automatically
                  </Text>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={styles.emptyButtonText}>View Patients</Text>
                  </TouchableOpacity>
                </View>
              )
            ) : (
              filteredBarcodes.map(renderBarcodeCard)
            )}
          </View>
        </View>

        {/* Generation Tip */}
        {recentBarcodes.length > 0 && !searchQuery.trim() && (
